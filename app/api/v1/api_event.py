...
@router.put('/{event_id}', dependencies=[Depends(login_required)])
def edit_event(event_id: int, request: EventCreateRequest, current_user: UserDetail = Depends(login_required),
               db: Session = Depends(deps.get_db)):
    event = event_srv.edit_event(event=request, event_id=event_id, user_id=current_user.id, db=db)
    return DataResponse().success_response(data=event)
...
def send_event_request(event_id: int, req_data: EventRequest, current_user: UserDetail = Depends(login_required),
                       db: Session = Depends(deps.get_db)):
    response = event_srv.send_event_request(db=db, event_id=event_id, user_ids=req_data.user_id,
